        ...
        {classes.map((session) => (
          <div key={session.id} className="flex items-center gap-3 rounded-lg border border-border p-3">
            <div className="w-11 text-center shrink-0">
              ...
              <p className="text-[10px] text-slate-400 tabular-nums">{session.endTime}</p>
            </div>
            <div className="w-px h-8 bg-slate-100 shrink-0" />
            <div className="min-w-0 flex-1">
              <p className="text-sm font-medium text-slate-800 truncate">{session.subject}</p>
              <p className="text-xs text-slate-500 truncate">{session.className}</p>
            </div>
